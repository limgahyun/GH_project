...
public class WeiService {
	...
	public void insertWeiGeneral(WeiRequest weiRequest, String path) throws Exception {
		...
		/*
		 * for(MultipartFile multipartFile : multipartFileList) { //16진수 유효아이디가 만들어진다?
		 * UUID uuid = UUID.randomUUID(); String filename = uuid.toString();
		 * logger.debug("String filename : " + filename);
		 * 
		 * //-문자를 찾아 없에고 문자열을 반환한다. filename = filename.replace("-", "");
		 * 
		 * //2.파일 확장자 //lastIndexOf()문자열에서 마지막 문자열을 반환한다. //getOriginalFilename() 올린 파일의
		 * 전체 이름 int doIndex = multipartFile.getOriginalFilename().lastIndexOf(".");
		 ...
		 * 
		 * //3.파일 컨텐트 타입 String fileType = multipartFile.getContentType();
		 * logger.info("String fileType : " + fileType);
		 * 
		 * //4.파일 사이즈 long fileSize = multipartFile.getSize();
		 * 
		 * //5.파일 저장(매개변수 path를 이용) File file = new File(path+filename+"."+fileExt);
		 * 
		 * try { multipartFile.transferTo(file); } catch (IllegalStateException e) {
		 * e.printStackTrace(); } catch (IOException e) { e.printStackTrace(); }
		 * 
		 * 
		 * FileWeiVO fileWeiVo = new FileWeiVO(); logger.info("fileWeiVo : " +
		 * fileWeiVo); fileWeiVo.setFileWeiName(filename);
		 * fileWeiVo.setFileWeiExt(fileExt); fileWeiVo.setFileWeiType(fileType);
		 * fileWeiVo.setFileWeiSize(doIndex);
		 * 
		 * weiVo.getFileWei().add(fileWeiVo); }
		 */
			weiDao.insertWeiGeneral(weiVo);
			logger.info("서비스서비스서서서서버시바섣비다서빋사ㅓ비다서 weiVo : " + weiVo);//여기까지 값이 넘어옴 2019.12.03.
			
			for(FileWeiVO fileWeiVo : weiVo.getFileWei()) {
				fileWeiVo.setWei_id(weiVo.getWei_id());
				fileWeiDao.insertFileWei(fileWeiVo);
				logger.info("서비스서비스서서서서버시바섣비다서빋사ㅓ비다서 fileWeiVo : " + fileWeiVo);
			}
	}
